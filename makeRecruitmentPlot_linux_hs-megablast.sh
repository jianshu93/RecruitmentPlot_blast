...
then
  ...
  ##blastn -db $database_all -query $reads -out $output/tmp.orig.blst -mt_mode 1 -num_threads $(nproc) -evalue 1e-9 -outfmt '6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen slen'
  hs-blastn -evalue 1e-9 -outfmt 6 -num_threads $(nproc) -out $output/tmp.orig.blst $reads $database_all 
